...
export function ResilientImage({ image, priority }: { image: PortfolioImage; priority: boolean }) {
  ...
  return <Image src={assetPath(image.src)} alt={image.alt} fill sizes="(max-width: 767px) 100vw, (max-width: 1023px) 50vw, 800px" preload={priority} className="object-contain" onError={() => setFailed(true)} />;
}
